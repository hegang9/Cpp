...
#### CLOSE_WAIT
- 进入条件：对端发送 FIN，说明对端不会再发送数据，本端收到 FIN 后进入 CLOSE_WAIT，此时本端还没有调用close()，连接还没有完全关闭。

#### CLOSE_WAIT 处理要点
- 及时调用 close() 释放资源，避免连接泄露
- 监控系统连接状态，发现大量 CLOSE_WAIT 可能是应用未正确关闭连接
- 对于长连接，考虑心跳机制及时发现死连接并关闭
- 对于服务端，通常是被动关闭方，进入 CLOSE_WAIT 后应尽快调用 close() 释放资源
- 对于客户端，通常是主动关闭方，进入 TIME_WAIT 后会自动释放资源，无需特殊处理
