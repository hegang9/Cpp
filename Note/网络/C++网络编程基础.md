...
#### 网络字节序转换（<arpa/inet.h>（Linux））
- htons()：将16位无符号短整型（如端口号）从主机字节序转换为网络字节序，设置端口号时使用
- htonl()：将32位无符号长整型（如IP地址）从主机字节序转换为网络字节序，设置IP地址时使用
- ntohs()：将16位无符号短整型从网络字节序转换回主机字节序，解析端口号时使用
- ntohl()：将32位无符号长整型从网络字节序转换回主机字节序，解析IP地址时使用

#### 网络编程中处理IP地址的功能函数
- **inet_pton**：将点分十进制或冒号分隔的IP地址字符串转换为网络字节序的二进制格式
    ```c
    #include <arpa/inet.h>
    ...
    - 参数说明：
      - af：地址族
      - src：指向包含IP地址字符串（如"192.168.1.1"或"2001:db8::1"）的指针
      - dst：指向存放转换后二进制结果的缓冲区指针。IPv4需至少4字节，IPv6需至少16字节
    - 返回值：
      - 1：成功
      - 0：输入字符串不是有效格式
      - -1：发生错误，设置errno
     
- **inet_ntop**：将网络字节序的二进制IP地址转换回字符串格式，是inet_pton的逆操作
    ```c
    const char *inet_ntop(int af, const void *src, char *dst, socklen_t size);​
    ```
    - 参数说明：
      - src：指向包含二进制IP地址的指针（如指向struct in_addr或struct in6_addr）
      - dst：指向目标字符串缓冲区的指针
      - size：目标缓冲区的大小，建议使用INET_ADDRSTRLEN（IPv4）或INET6_ADDRSTRLEN（IPv6）定义缓冲区
    - 返回值：成功时返回指向结果字符串的指针（即dst），失败返回NULL
- inet_addr与 inet_ntoa：传统的IPv4专用函数，已经过时，只做简单介绍。
    ```c
    in_addr_t inet_addr(const char *cp)
    ```
    - cp：点分十进制IPv4地址字符串
    - 返回值：成功时返回网络字节序的32位IPv4地址。如果输入字符串无效，则返回INADDR_NONE（通常是(in_addr_t)-1）
    ```c
    char *inet_ntoa(struct in_addr in)
    ```
    - in：一个包含网络字节序IPv4地址的struct in_addr结构体
    - 返回值：返回一个指向静态分配缓冲区的指针，该缓冲区包含点分十进制字符串。此函数非线程安全，且多次调用会覆盖上一次结果，如需保存应使用strcpy


### TCP粘包和拆包
#### 粘包
- 发生原因：
  1. 为了提高网络效率，TCP 可能使用 Nagle 算法将多个发出的小数据包在缓冲区中合并后一次性发送，相当于将多个不相干的数据包装箱打包；
  2. 接收方的内核缓冲区可能同时堆积了多个到达的数据包。如果应用层 recv时指定的缓冲区足够大，就可能一次性地将多个包的数据读取出来，造成粘包。

#### 拆包
- 发生原因：
  1. 如果单个应用层数据包太大，超过 MSS（最大报文段长度），TCP 也会将其拆分传输；
  2. 如果应用层缓冲区小于当前可读的数据量，则一个完整的包可能需要多次 recv调用才能读完，这就是拆包

#### 解决方案
1. 定长消息法：每个消息都严格采用固定长度，使用预定义填充符补全。会浪费带宽。
2. 分隔符法：在每个消息末尾添加一个特殊的字符或序列作为结束标记（例如换行符 \n，或 \r\n）。**适用于文本协议如HTTP**，但是消息体本身不能包含分隔符，必要时需要进行转义。**节省空间但需要转义**
3. 长度前缀法：在发送每个消息体之前，先发送一个固定长度的字段（通常是 4 字节的 uint32_t）来指明后续消息体的真实长度。接收端在接收缓冲区中先读取4字节，得到长度N，再读取N字节的数据。**最常用最灵活**。
4. 企业级协议设计：使用消息头对数据属性进行说明，解析时先解析消息头

### 八股提问热点
1. **TCP发生粘包和拆包的原因，UDP会发生粘包或拆包吗？**
TCP粘包和拆包原因见上，UDP不会发生粘包或拆包，因为UDP时面向数据报的协议，每个数据报都是独立的，且没有缓冲机制，每个包都是立即独立传输。但是UDP会发生丢包和乱序。

2. **如何选择粘包拆包解决方案？**
消息长度固定的场景，选择定长消息法；文本协议使用分隔符法；大多数场景可以使用长度前缀法；企业级应用使用自定义协议法。