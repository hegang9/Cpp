...
`unordered_map` 是 C++11 引入的关联容器，基于**哈希表**实现，存储键值对（key-value pair）。与 `map` 不同，`unordered_map` 不对元素进行排序，因此查找、插入和删除操作的平均时间复杂度为 **O(1)**。在C++标准库中，解决哈希冲突的方法是使用**链表法**（chaining），即每个桶中存储一个链表来处理哈希冲突。
